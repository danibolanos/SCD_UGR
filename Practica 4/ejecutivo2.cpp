...
int main( int argc, char *argv[] )
{
   ...
   time_point<steady_clock> actual;
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         sleep_until( ini_sec );
			actual = steady_clock::now();

			if(milliseconds_f(actual - ini_sec) > milliseconds(20)){
				cout << "Retraso por " << milliseconds_f(actual - ini_sec).count() << " milisegundos." << endl;
				return 0;
			}
